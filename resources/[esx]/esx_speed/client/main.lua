...
AddEventHandler('esx_speed:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_speed:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()

	if zone == 'speed' then
		TriggerServerEvent('esx_speed:stopHarvestKoda')
	elseif zone == 'ProcessamentoDeVinho' then
	TriggerServerEvent('esx_speed:stopTransformKoda')
	elseif zone == 'VendaDeBitcon' then
		TriggerServerEvent('esx_speed:stopSellKoda')
	end
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerServerEvent('esx_speed:GetUserInventory', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_speed:hasExitedMarker', lastZone)
		end
		if nasosta == 0 then
			waitara = 500
		end
...
-- itemsy
RegisterNetEvent('esx_speed:ReturnInventory')
AddEventHandler('esx_speed:ReturnInventory', function(kodaNbr, kodapNbr, jobName, currentZone)
	kodaQTE			= kodaNbr
	koda_poochQTE	= kodapNbr
	myJob			= jobName
	TriggerEvent('esx_speed:hasEnteredMarker', currentZone)
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
						...
						if k == "speed" then
							if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < Config.ZoneSize.x / 2) then
								...
								if torba == 40 or torba == 41 or torba == 44 or torba == 45 then
									TriggerServerEvent('esx_speed:startHarvestKoda', true)
								else
									TriggerServerEvent('esx_speed:startHarvestKoda', false)
								end
							else
								TriggerEvent('esx_speed:hasExitedMarker', lastZone)
							end
				...
				elseif CurrentAction == 'ProcessamentoDeVinho' then
					TriggerServerEvent('esx_speed:startTransformKoda')
				elseif CurrentAction == 'VendaDeBitcon' then
					for k,v in pairs(Config.Zones) do
						if k == "VendaDeBitcon" then
							if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < Config.ZoneSize.x / 2) then
								TriggerServerEvent('esx_speed:startSellKoda')
							else
								TriggerEvent('esx_speed:hasExitedMarker', lastZone)
							end
							break
