...
AddEventHandler('oruzje:dajgalicenca', function ()
...
AddEventHandler('weapon:piku2', function(zona, id)
	...
	if xPlayer.getMoney() >= 25000000 then
		xPlayer.removeMoney(25000000)
		local store = zona..id
		MySQL.Async.execute('UPDATE weashops2 SET `owner` = @identifier WHERE name = @store', {
			['@identifier'] = xPlayer.identifier,
			['@store'] = store
		})
		TriggerClientEvent('esx:showNotification', _source, "Kupili ste GunShop za $25000000")
		TriggerClientEvent("esx_gun:ReloadBlip", _source)
	else
		local missingMoney = 25000000 - xPlayer.getMoney()
...
RegisterServerEvent('esx_gun:DajFirmi')
AddEventHandler('esx_gun:DajFirmi', function(firma, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer.getMoney() >= amount then
		xPlayer.removeMoney(amount)
		DajFirmi(firma, amount)
	else
		TriggerClientEvent('esx:showNotification', _source, "Nemate dovoljno novca kod sebe!")
	end
end)
...
AddEventHandler('wesh:KuPi2', function(id, zone, ide)
	...
	if xPlayer.getMoney() >= 5000 then
		...
		TriggerClientEvent('esx:showNotification', _source, "Kupili ste extended spremnik za 5000$!")
		xPlayer.removeMoney(5000)
		DajFirmi(zone..ide, 5000)
	else
