...
AddEventHandler('skinchanger:loadClothes', function()
	Citizen.Wait(5000)
	ESX.TriggerServerCallback('esx_tattooshop:requestPlayerTattoos', function(tattooList)
		if tattooList then
			for k,v in pairs(tattooList) do
      ...
      function(data, menu)
      	...
      	if(data.current.value ~= nil) then
      		...
      		for i,k in pairs(tattoosList[data.current.value]) do
      			table.insert(elements, {label= "Tetovaza n°"..i.."	("..k.price..Config.MoneySymbol..")", value = i, price = k.price})
      		end

      		ESX.UI.Menu.Open(
				'default', GetCurrentResourceName(), 'Tattoos_Categories_menu',
				{
					title    = 'Tetovaze | '..currentLabel,
					align    = 'top-left',
					elements = elements,
				},
