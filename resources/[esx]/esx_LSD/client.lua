...
function PostaviPosao()
	...
	local npc = CreatePed(4, model, -1606.175539625, 2107.193623047, 68.003272857666, 40.00, false, true)
...
Citizen.CreateThread(function()
    ...
    while true do
			...
			if (GetDistanceBetweenCoords(-1606.175539625, 2107.193623047, 68.003272857666,  kordic.x,  kordic.y,  kordic.z,  true) <= 50.0) and naso == 0 then
				waitara = 0
				naso2 = 1
				DrawMarker(27, -1606.175539625, 2107.193623047, 68.003272857666, 0, 0, 0, 0, 0, 0, 2.25, 2.25, 1.0001, 0, 128, 0, 200, 0, 0, 0, 0)
			end
			
			local isInMarker  = false
			local currentZone = nil

			if(GetDistanceBetweenCoords(kordic, -1606.175539625, 2107.193623047, 68.003272857666, true) < 2.25) and naso == 0 then
				isInMarker  = true
				currentZone = "prodaja"
			end
					...
					if naso == 1 then
						...
						if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then
								...
								if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
									Draw3DText( Config.Processing.x, Config.Processing.y, Config.Processing.z , "~w~Proizvodnja LSDa~y~\nPritisnite [~b~E~y~] da krenete sa proizvodnjom LSDa",4,0.15,0.1)
