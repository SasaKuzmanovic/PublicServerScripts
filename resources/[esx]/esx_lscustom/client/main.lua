		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 200)
						TriggerServerEvent("lscs:kupiPeraje", GetEntityModel(vehicle), price, vehicle)
						TriggerServerEvent("DiscordBot:Mehanicari", GetPlayerName(PlayerId()).." je kupio dio za $"..price)
					elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
						price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 200)
						TriggerServerEvent("lscs:kupiPeraje", GetEntityModel(vehicle), price, vehicle)
						TriggerServerEvent("DiscordBot:Mehanicari", GetPlayerName(PlayerId()).." je kupio dio za $"..price)
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 200)
						TriggerServerEvent("lscs:kupiPeraje", GetEntityModel(vehicle), price, vehicle)
						TriggerServerEvent("DiscordBot:Mehanicari", GetPlayerName(PlayerId()).." je kupio dio za $"..price)
					else
			...
			if v.modType ~= nil then
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 200)
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price[1] / 200) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
	...
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic' and PlayerData.job.grade > 2) or Config.IsMechanicJobOnly == false then
					lsMenuIsShowed = true
