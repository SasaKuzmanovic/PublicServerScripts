...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_bar:requestDBItems', function(ShopItems)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				TriggerServerEvent('esx_bar:buyItem', data.current.item, data.current.value, zone)
			end
...
AddEventHandler('esx_bar:hasEnteredMarker', function(zone)
	CurrentAction     = 'shop_menu'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('esx_bar:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
RegisterNUICallback('purchase', function(data, cb)
	TriggerServerEvent('esx_bar:buyItem', data.item, data.count, data.loc)
	cb('ok')
end)
