...
function OpenMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'citizen_wear' then
			...
			GpsID = 1
			TriggerEvent("gepeke:OdjebiGa", GpsID)
...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) and zone.Foot then
			ESX.ShowNotification(_U('foot_work'))
		else
			if not zone.Foot then
				if IsPedInAnyVehicle(playerPed, false) then
					...
					if vehicle == VoziloTo then
						...
						ESX.ShowNotification("Niste u vozilu od posla!")
					end
				else
		...
		for jobKey,jobValues in pairs(Config.Jobs) do
			if jobKey == PlayerData.job.name then
				for zoneKey,zoneValues in pairs(jobValues.Zones) do
					if zoneValues.Id == id then
						TriggerEvent("gepeke:OdjebiGa", id)
						SetNewWaypoint(zoneValues.Pos.x, zoneValues.Pos.y)
						ESX.ShowNotification("Oznacena vam je iduca pozicija za posao!")
					end
				end
			end
		end
...
AddEventHandler('esx:setJob', function(job)
	...
	if PlayerData.job ~= nil then
		for jobKey,jobValues in pairs(Config.Jobs) do
			...
			if jobKey == PlayerData.job.name then
				for zoneKey,zoneValues in pairs(jobValues.Zones) do
					if zoneValues.Id == 1 then
						SetNewWaypoint(zoneValues.Pos.x, zoneValues.Pos.y)
						ESX.ShowNotification("Oznacena vam je pozicija posla!")
					end
				end
			end
		end
...
AddEventHandler('esx_jobs:spawnJobVehicle', function(spawnPoint, vehicle)
	...
	ESX.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)
		...
		Wait(200)
		VoziloTo = GetVehiclePedIsIn(playerPed, false)
