...
RegisterNetEvent('pullout:PostaviGa')
AddEventHandler('pullout:PostaviGa', function(ata)
	NemojAnim = ata
end)
		...
		if not NemojAnim then
			...
			if DoesEntityExist( ped ) and not IsEntityDead( ped ) and GetVehiclePedIsTryingToEnter(ped) == 0 and not IsPedInParachuteFreeFall (ped) then
				if CheckWeapon(ped) then
					if holstered or kurac ~= GetSelectedPedWeapon(PlayerPedId()) then
						blocked   = true
						TaskPlayAnim(ped, "reaction@intimidation@1h", "intro", 8.0, 2.0, -1, 48, 2, 0, 0, 0 )
						Citizen.Wait(cooldown)
						ClearPedTasks(ped)
						holstered = false
						kurac = GetSelectedPedWeapon(PlayerPedId())
					else
						blocked = false
					end
				else
