...
function refreshBlips()
	if playerData.job ~= nil then
		if playerData.job.name ~= nil and playerData.job.name == 'fork' then 	
			drawBlip(Config.locker, 280, "Skladistar ormar")			 	
			drawBlip(Config.carSpawner, 479, "Skladistar spawn vozila")					
			drawBlip(Config.carDelete, 490, "Skladistar placa")				
		end
	end
end
...
function spawnFork()							
	...
	SetVehicleNumberPlateText(currentCar, "CRO" .. math.random(1000, 9999))								
...
function spawnBox(coords)
	Citizen.CreateThread(function()
		...
		ESX.Game.SpawnLocalObject('prop_boxpile_06a', {
			x = coords.x,
			y = coords.y,
			z = coords.z
		}, function(obj)
...
function boxCanSpawn(coords)
	local object = GetClosestObjectOfType(coords.x,  coords.y,  coords.z,  3.0,  GetHashKey('prop_boxpile_06a'), false, false, false)
...
function goDeliver()
	ESX.ShowNotification('Dostavite paletu na oznaceno mjesto.')
	setGPS(taskPoints['deliver'])
	currentJob = 'deliver'
end

function goPickup()
	ESX.ShowNotification('Pokupite paletu na oznacenom mjestu')
...
function getPaid()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) and isMyCar() then			
		...
		TriggerServerEvent('esx_zlica:dobijTuljana', pay)
		ESX.ShowNotification('~w~Placa: ~g~ +' .. pay .. ' ~w~$.')
	else														
		ESX.ShowNotification('~r~Gdje je viljuskar?')
		local pay = math.floor(packetsDelivered * (Config.pay * 0.6))			
		if packetsDelivered < 2 then						
			pay = 0								
		end
		TriggerServerEvent('esx_zlica:dobijTuljana', pay)
		ESX.ShowNotification('~w~Placa: ~r~ +' .. pay .. ' ~w~$.')
	end
	currentJob = 'none'											
	currentPlate = ''												
	currentVehicle = nil											
	...
	while true do																
		...
		if not menuIsOpen then
			...
			elseif onDuty and currentJob == 'deliver' and taskPoints['deliver'] ~= nil and playerIsInside(playerCoords, taskPoints['deliver'], Config.pickupDistance) then
				isInMarker = true
				displayHint = true
				hintToDisplay = "Odvezite viljuskar na oznaceno mjesto."
				currentZone = 'none' 
			elseif playerData.job ~= nil and playerData.job.name == "fork" and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 1.5) then  				
				isInMarker = true
				displayHint = true
				hintToDisplay = "Pritisnite E za vracanje viljuskara."
				currentZone = 'pay'
			else																			
				isInMarker = false
				displayHint = false
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',			
    {
      title    = "Ormar",							
      elements = {
        {label = "Radna odjeca", value = 'fork_wear'},		
        {label = "Vasa odjeca", value = 'everyday_wear'}	
      }
    },
    function(data, menu)									
