...
Citizen.CreateThread(function ()
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'cardealer' then
			...
			if ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'vlasnik' then
			...
			if data2.current.value == 'test' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
					...
					if data4.current.value == 'yes' then
						if Config.EnablePlayerManagement then
							ESX.TriggerServerCallback('autosalon:sealionSociety', function(hasEnoughMoney)
							...
							if Config.EnableSocietyOwnedVehicles and (playerData.job.grade_name == 'boss' or  playerData.job.grade_name == 'vlasnik') then
								ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
									...
									if data3.current.value == 'personnal' then
										...
										ESX.TriggerServerCallback('autosalon:sealion', function(hasEnoughMoney)
											if hasEnoughMoney then
												IsInShopMenu = false
								...
								ESX.TriggerServerCallback('autosalon:sealion', function (hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Streaming.RequestModel(vehicleData.model)
										GarazaV = CreateVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos.x, Config.Zones.ShopOutside.Pos.y, Config.Zones.ShopOutside.Pos.z, Config.Zones.ShopOutside.Heading, true, false)
										TaskWarpPedIntoVehicle(playerPed, GarazaV, -1)
										SetVehicleDirtLevel(GarazaV, 0)
										local newPlate     = GeneratePlate()
										local vehicleProps = ESX.Game.GetVehicleProperties(GarazaV)
										vehicleProps.plate = newPlate
										SetVehicleNumberPlateText(GarazaV, newPlate)
										local props = ESX.Game.GetVehicleProperties(GarazaV)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:posaljiTuljana', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
AddEventHandler('esx_vehicleshop:hasEnteredMarker', function (zone)
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				ESX.TriggerServerCallback('garaza:JelIstiModel', function(dane)
					if (dane) then
						ESX.TriggerServerCallback('esx_vehicleshop:DajGaVamo', function(valja)
							if not valja then
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealer' and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'vlasnik') then
