...
AddEventHandler('ducan:piku2', function(zona, id)
	...
	if xPlayer.getMoney() >= 10000000 then
		xPlayer.removeMoney(10000000)
		local store = zona..id
		MySQL.Async.execute('UPDATE shops2 SET `owner` = @identifier WHERE store = @store', {
			['@identifier'] = xPlayer.identifier,
			['@store'] = store
		})
		TriggerClientEvent('esx:showNotification', _source, "Kupili ste trgovinu za $10000000")
		TriggerClientEvent("esx_shops:ReloadBlip", _source)
	else
		local missingMoney = 10000000 - xPlayer.getMoney()
...
RegisterServerEvent('esx_shops:DajFirmi')
AddEventHandler('esx_shops:DajFirmi', function(firma, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
