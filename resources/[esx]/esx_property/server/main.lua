...
function RemoveOwnedProperty(name, owner)
	...
	if xPlayer then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE owner = @owner AND name = @name', {
				['@owner'] = owner,
				['@name']  = name
		}, function(res)
			if res[1].rented == 0 then
				xPlayer.addMoney(math.ceil(prop.price/2))
			end
		end)
	end
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM properties', {}, function(properties)
		...
		for i=1, #properties, 1 do
			...
			if properties[i].auto ~= nil then
				auto = json.decode(properties[i].auto)
			end
...
RegisterServerEvent('prop:getTuljana')
AddEventHandler('prop:getTuljana', function(owner, type, item, count)
