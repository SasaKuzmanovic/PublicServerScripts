				...
				function(data2, menu2)
					if data2.current.value == 'tuning' then
						...
						if Distance < 5.0 then
							if Cijena[closestVehicle] ~= nil then
								if tonumber(Cijena[closestVehicle]) > 0 then
									local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
									if closestPlayer == -1 or closestDistance > 3.0 then
										ESX.ShowNotification(_U('no_players_nearby'))
									else
										menu2.close()
										ESX.ShowNotification("Dali ste racun u iznosu od "..Cijena[closestVehicle])
										TriggerServerEvent("DiscordBot:Mehanicari", GetPlayerName(PlayerId()).." je dao racun(tuning) igracu "..GetPlayerName(closestPlayer).." od $"..Cijena[closestVehicle])
										TriggerServerEvent('esx_billing:posaljiTuljana', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), Cijena[closestVehicle])
										Cijena[closestVehicle] = 0
									end
								end
							end
						end
