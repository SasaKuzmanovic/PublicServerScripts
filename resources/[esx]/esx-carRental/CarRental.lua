	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			...
			elseif WarMenu.Button('Banshee | Unaprijed: $1000 | Dan: $300') then
				...
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Elektricni romobil | Unaprijed: $25 | Dan: $50') then
				SpawnVehicle("serv_electricscooter")
				TriggerServerEvent("NaplatiTuljana", 25)
				ESX.ShowNotification("Naplaceno vam je $25 za iznajmljivanje romobila.")
				ESX.ShowNotification("Da vratite vozilo upisite /unrent")
				autoChargeAmount = 50
				isBeingCharged = true
				WarMenu.CloseMenu()
