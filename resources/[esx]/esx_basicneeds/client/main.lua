...
AddEventHandler('esx_basicneeds:resetStatus', function()
	...
	TriggerEvent('esx_status:set', 'shit', 500000)
...
AddEventHandler('esx_basicneeds:healPlayer', function()
	...
	TriggerEvent('esx_status:set', 'shit', 1000000)
	...
	Citizen.CreateThread(function()
		while true do
			...
			TriggerEvent('esx_status:getStatus', 'pee', function(status)
				if tonumber(status.val) <= 50000 then
			...
			TriggerEvent('esx_status:getStatus', 'drunk', function(status)
				if status.val > 0 then
					...
					local level = -1
					if status.val > 300000 and status.val <= 450000 then
						level = 0
					elseif status.val <= 600000 then
						level = 1
					else
						level = 2
					end
