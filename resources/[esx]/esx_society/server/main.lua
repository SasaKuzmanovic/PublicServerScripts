...
RegisterServerEvent('drustvo:tuljanpovlacipare')
AddEventHandler('drustvo:tuljanpovlacipare', function(society, amount)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT name, firstname, lastname, identifier, job, job_grade FROM users WHERE job = @job ORDER BY job_grade DESC', {
			['@job'] = society
		}, function (results)
			local employees = {}
			...
			for i=1, #results, 1 do
				local imee = nil
				if results[i].name == nil then
					imee = "Retardirano_Ime"
				else
					imee = results[i].name
				end
				table.insert(employees, {
					name       = imee.." ("..results[i].firstname .. ' ' .. results[i].lastname..")",
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = (xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'vlasnik')
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and (xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'vlasnik') then
		return true
	else
