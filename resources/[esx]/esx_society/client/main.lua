		...
		if data.current.value == 'withdraw_society_money' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
				...
				if amount == nil then
					...
					menu.close()
					TriggerServerEvent('drustvo:tuljanpovlacipare', society, amount)
				end
...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		for i=1, #employees, 1 do
			...
			if ESX.PlayerData.job.grade_name ~= 'vlasnik' then
				if employees[i].job.grade_name ~= "boss" and employees[i].job.grade_name ~= "vlasnik" then
				...
				if employees[i].job.grade_name ~= "vlasnik" then
					table.insert(elements.rows, {
							data = employees[i],
							cols = {
								employees[i].name,
...
function OpenPromoteMenu(society, employee)
	...
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		for i=1, #job.grades, 1 do
			...
			if ESX.PlayerData.job.grade_name ~= 'vlasnik' then
				if job.grades[i].name ~= "boss" and job.grades[i].name ~= "vlasnik" then
					table.insert(elements, {
						label = gradeLabel,
				...
				if job.grades[i].name ~= "vlasnik" then
					table.insert(elements, {
							label = gradeLabel,
							value = job.grades[i].grade,
							selected = (employee.job.grade == job.grades[i].grade)
