...
function SpawnajGa(race)
	...
	if ucitao == 0 then
		...
		RaceVehicle = CreateVehicle(GetHashKey(raceInfo['Vehicle']), Rx, Ry, Rz, Rh, true, false)
		...
		if Lokacija == 0 then
			...
			if Prvi == 0 then
				...
				Lokacija = 0
				TriggerServerEvent("SpremiPomocne", Lokacija, ucitao, Prvi)
				RaceVehicle = CreateVehicle(GetHashKey(raceInfo['Vehicle']), Rx2, Ry2, Rz2, Rh2, true, false)
				FreezeEntityPosition(RaceVehicle, true)
				local cordsa = GetOffsetFromEntityInWorldCoords(RaceVehicle, 0.0, -6.0 , 0.0)
				Rx2 = cordsa.x
				Ry2 = cordsa.y
