...
------
RegisterNetEvent('InteractSounda_CL:PlayWithinDistance')
AddEventHandler('InteractSounda_CL:PlayWithinDistance', function(playerNetId, maxDistance, soundFile, soundVolume)
    local lCoords = GetEntityCoords(GetPlayerPed(-1))
    local eCoords = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(playerNetId)))
    local distIs  = Vdist(lCoords.x, lCoords.y, lCoords.z, eCoords.x, eCoords.y, eCoords.z)
    if(distIs <= maxDistance) then
        SendNUIMessage({
            transactionTypea     = 'playSounda',
            transactionFilea     = soundFile,
            transactionVolumea   = soundVolume
        })
    end
end)
