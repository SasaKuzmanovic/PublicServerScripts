...
RegisterServerEvent('ProdajDrogeTuljanu')
AddEventHandler('ProdajDrogeTuljanu', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local weedqty = 0
	local weedqtySingle = xPlayer.getInventoryItem('marijuana').count
	local methqty = 0
	local methqtySingle = xPlayer.getInventoryItem('cocaine').count
	local cokeqty = 0
	local cokeqtySingle = 0
	local opiuqty = 0
	local opiuqtySingle = 0
	local x = 0
	local blackMoney = 0
	local drugType = nil
	...
	elseif drugType=='marijuana' then	--single
	...
	TriggerClientEvent('esx:showNotification', _source, "Prodali ste " .. '~b~'..x..'g~w~ ' .. Kura .." za ".. blackMoney .. '$')
end)
...
AddEventHandler('drugsInProgress', function(street1, street2, sex)
	...
	TriggerClientEvent("outlawNotify", -1, "~r~Osoba ~w~"..sex.." ~r~ spola prodaje drogu izmedju ~w~"..street1.."~r~ i ~w~"..street2)
end)
...
AddEventHandler('drugsInProgressS1', function(street1, sex)
	...
	TriggerClientEvent("outlawNotify", -1, "~r~Osoba ~w~"..sex.." ~r~spola prodaje drogu u ulici ~w~"..street1)
end)
