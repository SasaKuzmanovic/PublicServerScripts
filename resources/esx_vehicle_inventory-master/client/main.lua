				...
				if (not IsTrunkOpen) then
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
						...
						if locked == 1 or class == 15 or class == 16 or class == 14 then
							...
							ESX.ShowNotification('Gepek ovog vozila je ~r~zatvoren')
						end
					end
				else 
					ESX.ShowNotification('~r~Netko vec gleda u ovaj gepek!')
				end
			end, targa)
		else
			ESX.ShowNotification('Nemate ~r~vozila~w~ u blizini')
		end
		lastOpen = true
...
--[[
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('Trenutna kilaza gepeka : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
              else
                ESX.ShowNotification('Vas gepek je pun:  ~r~ '..MaxVh..' Kg')
              end
				    else
			      		ESX.ShowNotification('~r~ Netocna kolicina')
				    end
          ...
          --fin test
            ...
            if not max then
              ...
              ESX.ShowNotification('~r~ Pretesko za gepek ovog vozila')
            end
			    else
			      ESX.ShowNotification('~r~ Netocna kolicina')
			    end
