    ...
    for k, _ in pairs(listSrc) do
      getPhoneNumber(tonumber(k), function (n)
        if n ~= nil then
          ...
          TriggerEvent('gcPhone:_internalAddMessage', number, n, 'Od #' .. alert.numero  .. ' : ' .. alert.message, 0, function (smsMess)
...
AddEventHandler('esx_phone:registerNumber', function(number, type, sharePos, hasDispatch, hideNumber, hidePosIfAnon)
  print('==== Registriran broj za ' .. number .. ' => ' .. type)
...
AddEventHandler('esx_addons_gcphone:startCall', function (number, message, coords)
  ...
  if PhoneNumbers[number] ~= nil then
    ...
    print('Zove nepostojeci servis => Broj : ' .. number)
  end
end)
...
AddEventHandler('esx_phone:send', function(number, message, _, coords)
  ...
  if PhoneNumbers[number] ~= nil then
    ...
    -- print('esx_phone:send | Calls on a non-registered service => numero : ' .. number)
  end
end)
